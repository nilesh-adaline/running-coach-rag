 ...
 */
async function assembleAugmentedPrompt(trace, coachTemplate, userQuery) {
    ...
    const matches = await (0, retrieve_1.retrieveTopK)(5, trace, fullPrompt, parentReferenceId);
    console.log(`Retrieved ${matches.length} match(es)`);
    for (let i = 0; i < matches.length; i++) {
        const m = matches[i];
        const { fileName, chunkNum } = await (0, retrieve_1.parseMatchMetadata)(m);
        if (fileName && typeof chunkNum === 'number') {
            const content = await (0, retrieve_1.readChunkContent)(fileName, chunkNum);
 ...
 */
async function callLLM(systemMessage, userMessage, trace) {
    const startTime = Date.now();
    const apiKey = process.env.OAI_API_KEY;
    if (!apiKey)
        throw new Error('OAI_API_KEY missing');
    // Pull dynamic model/config from deployment payload
    const info = await (0, fetchDeployedPrompt_1.getDeploymentInfo)(trace);
    const providerName = info.providerName;
    const modelName = info.model;
    const config = (0, types_1.Config)().parse(info.settings);
    ...
    try {
        ...
        let usageObj = null;
        const hasUsage = !!(usage && (usage.inputTokens !== undefined ||
        ...
        completionDetails = {
            ...
            choices: resp?.response?.choices || [{
                    ...
                    message: {
                        ...
                        content: outputMessage?.content?.find((c) => c.modality === 'text')?.value || '',
                        refusal: null,
                    },
                    logprobs: null,
        ...
        const llmCost = (inputTokensForCost / 1000000) * inputCostPer1M + (outputTokensForCost / 1000000) * outputCostPer1M;
        ...
        const variables = {};
        for (const [key, val] of Object.entries(fetchDeployedPrompt_1.DEFAULT_QUERY_VARIABLES)) {
            variables[key] = {
                modality: 'text',
                value: String(val),
            };
    ...
    const textContent = assistantMsg?.content?.find((c) => c.modality === 'text' && c.value);
...
async function main() {
    ...
    const trace = (0, observability_1.createTrace)('The Running Coach App (RAG)');
    try {
        // 2. Fetch prompt first to enforce ordering: fetch_deployed_prompt -> prompt_retrieval
        const { coachTemplate, userQuery } = await (0, prompt_1.getFullPrompt)(trace);
