...
exports.submitTrace = submitTrace;
...
function createTrace(name, projectId) {
    return {
        name,
...
function addSpan(trace, span) {
    const inferredType = span.content?.type;
    const provider = span.content?.provider;
    const model = span.content?.model;
    const newSpan = {
        ...span,
...
async function submitTrace(trace) {
    if (!ADALINE_API_KEY) {
    ...
    const mapStatus = (s) => (s === 'success' ? 'success' : 'failure');
    // Prepare payload per POST /v2/logs/trace spec
    const projectId = trace.projectId || process.env.ADALINE_PROJECT_ID || fetchDeployedPrompt_1.PROJECT_ID;
    ...
    const spansPayload = trace.spans.map((s) => {
        ...
        if (type === 'Model') {
            const modelFromInput = (typeof c.input === 'object' && c.input) ? c.input.model : undefined;
