...
exports.projectToDim = projectToDim;
...
exports.parseMatchMetadata = parseMatchMetadata;
...
async function createQueryEmbedding(text, trace, parentReferenceId) {
    const startTime = Date.now();
    const apiKey = process.env.OAI_API_KEY;
    if (!apiKey)
        throw new Error('OAI_API_KEY is required in the environment');
    ...
    try {
        ...
        if (!items || items.length === 0)
            throw new Error('No embeddings returned from gateway');
        const emb = items[0].embedding;
...
async function retrieveTopK(k = 5, trace, fullPromptOverride, parentReferenceId) {
    let fullPrompt = fullPromptOverride;
    if (!fullPrompt) {
        const coach = await (0, prompt_1.getCoachTemplate)();
        const userQuery = await (0, prompt_1.getUserQuery)();
        fullPrompt = `${coach}\n\nUser request:\n${userQuery}`;
    }
    ...
    try {
        const results = await index.query({ vector: qemb, topK: k, includeMetadata: true });
            ...
            (0, observability_1.addSpan)(trace, {
                ...
                content: {
                    ...
                    input: {
                        ...
                        query: fullPrompt,
                        queryLength: fullPrompt.length,
                        queryWordCount: fullPrompt.split(/\s+/).length,
                        topK: k,
...
async function readChunkContent(fileName, chunkNum, chunkSize = 800) {
    try {
        ...
        if (fileName.toLowerCase().endsWith('.pdf')) {
            ...
            try {
                ...
                doc = (await (0, promises_1.readFile)(filePath, 'utf8')).replace(/\s+/g, ' ').trim();
            }
        }
        else {
...
async function parseMatchMetadata(match) {
    let fileName;
    let chunkNum;
    if (match.metadata) {
        fileName = (match.metadata.file ?? match.metadata.source ?? match.metadata.filename);
        chunkNum = (match.metadata.chunk ?? match.metadata.chunkIndex ?? match.metadata.chunk_num);
    }
    if ((!fileName || chunkNum === undefined) && match.id) {
        ...
        if (idMatch) {
            ...
            try {
                ...
                const files = await (0, promises_1.readdir)(dataDir);
                const found = files.find((f) => path_1.default.basename(f, path_1.default.extname(f)) === base || f.startsWith(base));
                if (found)
                    fileName = found;
            }
            catch {
